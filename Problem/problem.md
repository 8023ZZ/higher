...
2. kafka 解耦扣款接口
   行内购买接口响应非常慢，通常需要7到10s，会导致用户等待，然后请求全部积压到后端，后端全部阻塞等待，可能会导致其他接口无法访问，占用内存过高。因此引入 kafka 解耦购买请求和接口调用。新建1个 topic， 5个partition， 5个定时任务轮询消费作为消费者。使用最大努力通知方案
   为了避免重复消费，保证消息幂等性，kafka 中每个key 为机器ip加时间戳加随机数，消费者线程获取到消息后，通过 redis 判断 key 是否已被处理（SADD），记录 key 为处理中，调用消费接口完成后再记录key为已完成处理，失败就进行重试，重试五次全部失败或者业务逻辑成功手动提交 offset
   为了避免消息丢失，设置生产者 ack 为 -1，等待 kafka 集群同步完成，如果超时未 ack 则重试
   每日解析第三方的对账文件，将最终未成功的订单再次投递

### 保险购买项目亮点
1. 解耦流程模块
   用户行为回溯视频包含采集和生成视频两部分，采集主要是埋点用户行为操作传入后端进行记表，为了减少频繁调用，会先在前端缓存，提交时调用后端接口上送
   视频生成流程包含H5页面还原、H5 to 图片、图片合成为视频三个流程，每个流程处理速度不一样，为了提升效率，拆分成2个线程池分别进行处理，通过 CountDownLatch 来控制内部子线程的等待点，外部线程池设置为4个线程，内部IO密集型业务线程池设置为6个线程
2. JVM 优化
   行内反馈系统崩溃无法提供服务，查询日志发现 java.lang.OutOfMemoryError:Java heap space，配置有 -XX:+HeapDumpOnOutOfMemoryError 拿到自动生成的 dump 文件，通过 MAT 分析内存中有大量的 byte[] 数组，占据了 8 G内存，而我们分配的 JVM 内存也就是 8G，通过 MAT 工具查看引用发现是 Tomcat 的线程创建的，进一步发现 Tomcat 的工作线程大概有 400 个，每个工作线程创建了两个 byte[] 数组，每个数组大小为 10MB，每个数组大小是 max-http-header-size: 10000000 控制的。但是检查系统监控发现，每秒请求只有100而不是400，那就是每个线程需要 4s 才能处理完成。翻看事故发生前的日志，发现大量的超时，配置的超时时间是4s，因此问题的根本原因是 4s 的请求量积压导致的，所以将超时时间改为 1s，同时把 max-http-header-size 适当调小一些

### 数字化运营项目亮点
1. Redis 分布式锁
   redis 分布式锁避免库存超发
2. SQL 优化
   系统大量慢查询导致数据库链接数打满，用户系统本身TPS在几千，监控发现每分钟慢查询超过10W，出问题的sql大概是 select * from users where catagory = 'xx' and sub_catagory = 'xx' order by id desc limit xx,xx

   用户表大概是一亿的数据量，这个量级很稳定，但是该语句执行需要几十秒钟，正常来说这个sql 执行速度应该在一秒以内
   ...
   但是第三方系统调用时加了几种新的标签，但是这些标签在用户表里并没有对应的商品，所以底层在扫描聚簇索引时相当于全表扫描，导致慢查询出现，进而数据库连接池打满

3. 长事务解耦
   查询库存，加分布式锁，计算奖品，扣减库存，记表，调用第三方接口，事务很长，因此扣减库存后剩下的放 kafka，后续由消费者线程处理第三方接口调用的工作，使用最大努力通知方案
...
### 堆外内存溢出
公司调用第三方固定格式接口时有开发同事创建 directByteBuffer 未释放
...
同时由于内存设置比例不合理，年轻代只有一两百 M，而老年代有七八百 M，每次 GC 会有大概8M 的对象直接进入老年代，导致 DirectByteBuffer 直接进入了老年代，而又一直没触发 Full GC 导致

因此优化策略就是调整年轻带大小为 500 M
### SQL 查询调优
用户千万级，日活用户百万级
