...
内连接 on 和 where 等价，判断的结果集是一样的，左右连接以主表为主，on条件不对主表进行筛选
...
Using Where 代表 mysql 在存储引擎检索行后再进行过滤。许多 where 条件中设计索引中的列，当读取索引时，就能被存储引擎校验，因此不是所有 where 子句都会显示 Using Where 

Using temporary 表示 mysql 对查询结果排序时会使用临时表

Using filesort 表示 mysql会对结果使用一个外部索引排序，而不是按索引次序从表里读取行。mysql有两种文件排序算法，这两种排序方式都可以在内存或者磁盘上完成，explain不会告诉你mysql将使用哪一种文件排序，也不会告诉你排序会在内存里还是磁盘上完成。

### 主从架构
主从复制结构，一个是主节点 master，另一个是从节点 slave

系统平时通过 master 节点读写数据，master 节点会把数据自动复制到 slave 节点中，主从复制是基于bin log，从库有一个 IO 线程会和主库建立起一个 TCP 连接，请求主库将 bin log 传输给自己，加载进 relay 日志文件，然后会有另一个 SQL 线程会读取 retry 日志文件，把 sql 在从库中执行一遍

然后就可以部署成读写分离，master 节点处理写请求，slave 节点处理读请求，提升 QPS

主从延迟的问题，需要尽可能缩小主从同步的延迟时间，可以让从库也用多线程并行复制数据，另外可以强制设定从主库进行读取来避免主从延迟

### 分库分表
建议 Mysql 单表数据量不超过 1000W，最好在 500W 以内，最佳是 100W 以内

一般一亿行的数据，大致在1G到几个GB之间

水平拆分，垂直拆分（较少的访问频率高的字段放一起，较多的访问频率低的字段放一起）

另外还有两种分库分表的方式：

按 range 去拆分，一般按照时间范围，但是这样容易出现热点问题

按某个字段的 hash 去拆分，可以平均分配每个库的数据量和压力，坏处是扩容比较麻烦，会有数据迁移的过程，之前的数据需要重新计算 hash 重新分配

动态切换分库分表的方案：双写迁移方案

所谓双写，就是原来对于老库的增删改，都加上对新库的增删改，同时写两个库

系统部署之后，使用导数工具，跑起来读老数据库的数据写入新库，写的时候要根据 gmt_modified 这类字段判断这条数据的最后修改时间，除非读出来的数据在新库中没有，或者比新库新才会写，简单地说就是不允许用老数据覆盖新数据
...
所以容量一开始就设置为32个库，每个库32张表，一共1024张表，可以承担 32 * 1000 的 QPS
...
#### 基于数据库
数据库自增 ID：这个就是说你的系统里每次得到一个 id，都是往一个库的一个表里插入一条没什么业务含义的数据，然后获取一个数据库自增的一个 id。拿到这个 id 之后再往对应的分库分表里去写入。适合并发不高，但是数据量太大导致的分库分表扩容，你可以用这个方案，因为可能每秒最高并发最多就几百，那么就走单独的一个库和表生成自增主键即可。

设置数据库 Sequence 或者表自增字段步长：比如说，现在有 8 个服务节点，每个服务节点使用一个 sequence 功能来产生 ID，每个 sequence 的起始 ID 不同，并且依次递增，步长都是 8。适合在用户防止产生的 ID 重复时，这种方案实现起来比较简单，也能达到性能目标。但是服务节点固定，步长也固定，将来如果还要增加服务节点，就不好搞了。
...
雪花算法：是把一个 64 位的 long 型的 id，1 个 bit 是不用的，用其中的 41 bit 作为毫秒数，用 10 bit 作为工作机器 id，12 bit 作为序列号。
...
Mysql 主从复制有两个机制，一个是半同步复制，用来解决主库数据丢失的问题，另一个是并行复制，用来解决主从同步延时的问题

半同步复制，也叫 semi-sync 复制，指的就是主库写入 binlog 日志之后，就会将强制此时立即将数据同步到从库，从库将日志写入自己本地的 relay log 之后，接着会返回一个 ack 给主库，主库接收到至少一个从库的 ack 之后才会认为写操作完成了。

并行复制，指的是从库开启多个线程，并行读取 relay log 中不同库的日志，然后并行重放不同库的日志，这是库级别（schema）的并行。2000 TPS 情况下，主从复制的延迟大概是几十毫秒

如果主从复制延迟比较严重，一般有以下解决方案：
