...
方法区是一个 JVM 规范，永久代与元空间都是其一种实现方式。在 JDK 1.8 之后，原来永久代的数据被分到了堆和元空间中。元空间存储类的元信息，class对象、静态变量和常量池等放入堆中。
...
CMS 垃圾收集器采取的是垃圾回收线程和系统工作线程尽量同时执行的模式来处理的。
...
* 筛选回收：首先对各个 Region 中的回收价值和成本进行排序，根据用户所期望的 GC 停顿时间来制定回收计划。此阶段其实也可以做到与用户程序一起并发执行，但是因为只回收一部分 Region，时间是用户可控制的，而且停顿用户线程将大幅度提高收集效率。同时可以执行多次混合回收（默认值为8次），来保证尽可能缩短系统停顿时间。混合回收是基于复制算法，而空闲的 Region 数量达到堆内存 5% 时会停止混合回收，本次混合回收结束
...
##### 3. 长期存活对象进入老年代
为对象定义年龄计数器，对象在 Eden 出生并经过 Minor GC 依然存活，将移动到 Survivor 中，年龄增加一岁，增加到一定年限就移入老年代中

##### 4. 动态对象年龄判断
虚拟机并不是永远要求对象的年龄必须达到 MaxTenuringThreshold 才能晋升老年代，如果在 Survivor 中一个年龄及低于这个年龄的对象大小的总和大于等于 Survivor 空间的一半，则大于或等于该年龄的对象可以直接进入老年代，无需等到 MaxTenuringThreshold 中要求的年龄。

Minor GC 过后，要转移存活对象到 S 区之前触发，从最小年龄开始累加，直到某个年龄时，内存占比超过 50%，那么所有大于或等于这个年龄的对象就会被分配到老年代，然后会把 Eden 区和 S 区的剩余存活对象放到空闲的那个 S 区中。而且首次 GC 后不会触发，在下次 GC 后如果还大于 50% 才触发

Survivor 区的对象年龄从小到大进行累加，当累加到 X 年龄时总和大于 50%（可以用 -XX:TargetSurvivorRatio=? 来设置），那么比 X 大的都会晋入老年代，动态年龄判断发生在 Minor GC 前，在转移 eden 或者 s区 对象到 s区 时来判断存货对象是否满足

##### 5. 空间分配担保
在发生 Minor GC 之前，虚拟机先检查老年代最大可用的连续空间是否大于新生代所有对象的总空间，如果条件成立的话，那么 Minor GC 可以确认是安全的。

如果不成立的话虚拟机会查看 HandlePromotionFailure 的值是否允许担保失败，如果允许那么就会继续检查老年代最大可用的连续空间是否大于历次晋升到老年代对象的平均大小，如果大于，将尝试着进行一次 Minor GC；如果小于，或者 HandlePromotionFailure 的值不允许冒险，那么就要进行一次 Full GC，如果 Full GC 后老年代空间依然不够则 OOM

JDK7及以后这个参数就失效了，只要老年代的连续空间大于新生代对象的总大小或者历次晋升到老年代的对象的平均大小就进行MinorGC(两个条件满足一个即可)，否则FullGC。

#### Full GC
Minor GC 触发条件很简单，Eden 区满时就将触发一次 Minor GC，而 Full GC 触发有以下条件：
##### 1.调用 System.gc()
只是建议虚拟机执行 Full GC，但是虚拟机不一定真正去执行。不建议使用这种方式，而是让虚拟机管理内存。

##### 2. 老年代空间不足
老年代空间不足的常见场景是大对象直接进入老年代、长期存活的对象进入老年代等，以及 -XX:CMSInitiatingOccupancyFaction 参数到达设定比例阈值

为了避免以上原因引起的Full GC，应当尽量不创建大的对象和数组，除此之外，可以通过 -Xmn 虚拟机参数调大新生代的大小，让对象尽量在新生代被回收掉，不进入老年代。还可以通过 -XX:MaxTenuringThreshold 调大对象进入老年代的年龄，让对象在新生代多存活一段时间。

##### 3. 空间分配担保失败
使用复制算法的 Minor GC 需要老年代的内存空间来做担保，如果担保失败就会执行一次 Full GC
...
##### 5. Concurrent Model Failure
执行 CMS GC 的过程中同时有对象要放入老年代，而此时老年代空间不足（可能是 GC 过程中浮动垃圾过多导致暂时性内存空间不足），便会报 Concurrent Mode Failure 错误，并触发 Full GC

##### 6. 触发参数阈值
老年代连续最大可用内存大于历次进入老年代对象的平均内存，即通过了空间内存担保，但是老年代已经使用的内存空间占比大于参数指定的比例，则会触发 Full GC

#### 永久代（方法区/元空间）垃圾回收时机
* 该类的所有实例对象都已经从 Java 堆内存中被回收
* 加载这个类的 ClassLoader 已经被回收
* 对该类的 Class 对象没有任何被其他变量引用（因为拿到类的 class 对象就可以通过反射来创建对象）

JVM 的优化就是尽可能让对象都在新生代里分配和回收，尽量别让太多对象频繁进入老年代，尽量避免 Major GC，同时给系统充足的内存大小，避免频繁的 Minor GC

### 类加载机制
类在运行期间第一次使用时动态加载，而不是一次性加载所有类，如果一次性加载会占用很多内存
#### 类生命周期
![avatar](../static/类生命周期.png)
...
##### 3. 准备
类变量是被 static 修饰的变量，准备阶段为类变量(static 变量)分配内存并设置初始值，使用的是方法区的内存，而常量在编译阶段就已经被赋值

实例变量不会在这个阶段分配内存，它会在对象实例化时随着对象一起分配到堆内存中。实例化不是类加载的过程，类加载发生在所有实例化操作之前并且类加载只进行一次，实例化可以进行多次

初始值一般为 0 值，如果类变量是常量，那么它将初始化为表达式所定义的值而不是 0。
...
##### 5. 初始化
初始化阶段才开始真正执行类中定义的 Java 程序代码，初始化阶段是虚拟机执行类构造器 <clinit\>() 方法的过程，执行 java 的初始化代码

如果初始化一个类时发现它的父类还没初始化，则先初始化他的父类

静态语句块只能访问到定义在它之前的类变量，定义在它之后的类变量只能赋值，不能访问

#### 类初始化时机
##### 1. 主动引用
虚拟机规范中并没有强制约束何时进行加载，但是规范严格规定了有且只有以下五种情况必须对类进行初始化（加载、验证、准备、解析都会随之发生）：
...
#### JVM 运行状况
jstat -gc PID： 查看 JVM 进程的内存和 GC 情况，然后查看新生代对象的增长速度，YGC 的触发频率和每次耗时，每次 YGC 后有多少对象存活和进入老年代，FGC 的触发时机和耗时

#### 线上系统对象分布
jmap 可以了解系统运行时的内存区域

jmap -heap PID 会打印堆内存的参数设置，但一般用 jstat 即可

jmap -histo PID 会按照各种对象占用内存的大小排序

jmap -dump:live,format=b,file=dump.hprof PID 会生成 dump.hprof，即堆内存转储快照

jhat dump.hprof -port 7000 图形化页面分析堆转储快照
