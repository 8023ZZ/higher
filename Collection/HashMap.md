...
jdk1.8修改为尾插法后修复了这个问题，但依然会有数据成环的问题。

## HashMap底层的数据结构
对key做hash，然后按数组大小取模
最核心的数据结构是数组，数组+链表/红黑树

## 为什么HashMap数组扩容是2的倍数
数组长度为2的n次方的情况下： hash & (n - 1) == hash 对 n 取模
2的倍数减1得到值的所有位都是1，和计算的hash值相与后能保证结果单一，可以减少hash碰撞概率

## JDK1.8中对hash算法和寻址算法是如何优化的
hash算法优化,和寻址算法是匹配的
``` java
//JDK1.8以后的hashmap的hash算法源码
static final int hash(Object key) {
    int h;
    return (key == null) ? 0 : (h = key.hashCode()) ^ (h >>> 16);
}
```
key的hash值和hash值右移16位的值做异或，即高16位和0异或后作为高16位，高16位和低16位异或后做为低16位，然后转成int后返回

寻址算法优化：
（n - 1） & hash -> 数组里的一个位置（n为2的n次方）
不直接取模，是因为取模运算性能较差，远低于与运算性能

总结：
hash算法的优化：对每个hash值，在他的低16位中，让高低16位进行了异或，让他的低16位同时保持了高低16位的特征，尽量避免一些hash冲突，大家可能会进入数组的同一个位置。如果不进行优化，高16位不同但是低16位相同时，与运算结果是一致的，要避免这种情况
寻址算法的优化：用与运算替代取模，提升了性能

关键一点：n通常很小（hashmap长度），但是hash值是32位，n-1大概率是高位补0，因此大部分情况下hash的高16位不参与运算，所以对hash算法进行优化，让低16位同时具备高低16位的特征，减少hash冲突

## HashMap如何解决hash碰撞的
链表，链表长度大于8时且数组长度已经经过扩容满足转化成树的长度64时，把链表转为红黑树：O(n) 优化为 O(log n)，红黑树的空间复杂度较高。如果链表长度大于8但是数组长度小于64，那么会先进行扩容
8是通过泊松分布计算来的，链表长度为8的概率是千万分之一

## HashMap是如何进行扩容的
默认为2倍扩容，进行rehash，重新对每个hash进行寻址，判断二进制结果中是否多出一个bit的1，即新结果=老结果+老数组长度，如果多出来，就是newIndex = oldIndex + oldCap，通过这个方式，避免rehash的时候，用每个hash值对新数组长度进行取模操作
新的位置一定为老位置或者老位置index加上老数组长度